...
[Twigs](https://twigs.surveysparrow.com)  is a design library for developers to build apps on top of SurveySparrow. With pre-built components, Twigs offers control and flexibility to build rich interfaces consistent with SurveySparrowâ€™s product design and experience.
...
Letâ€™s take a closer look at the files that were created in the app.
...
As apps are rendered in an IFrame, the app and the underlying page communicate through methods by using the client object. The ``` client.data.get(â€˜getSurveyIdâ€™) ``` method uses a Data method to retrieve the surveyId in the integration page at survey level.
